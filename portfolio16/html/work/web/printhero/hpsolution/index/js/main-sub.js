...
KNBUI.keyVisual = (function ($) {
  ...
  function visual() {
    ...
    var visual = $('.visual'),
      ctrlBtn = $('.visual-controller li'),
      visualList = $('.visual-list li'),
      mainText2 = $('.main-text2 li'), // added
      counterText2 = $('.counter-text2'), // added
      btnStop = $('.btn-stop'),
      btnPlay = $('.btn-play'),
      ...
      autoPlay = true, // 자동재생
      start = 2, // 시작설정 0부터 시작
      playSpeed = 15000; // 단위 1000/1초

    function show(num) {
      ctrlBtn.eq(num).addClass('active');
      visualList.eq(num).fadeIn().addClass('active');
      mainText2.eq(num).fadeIn().addClass('active'); // added
      counterText2.eq(num).fadeIn(); // added
    }
    ...
    function clear(num) {
      ...
      mainText2.eq(num).fadeOut().removeClass('active'); // added
      counterText2.eq(num).fadeOut(); // added
    }
