(function(){
    ...
    function loading(){
        ...
        function updateProgress(){
            ...
            if(imgCurrent >= 100) {
                ...
                .to("#header .menu-line", {x:0, duration:1},"-=1");
    ...
    navLi[2].firstChild.addEventListener('click',function(e){
        //e.preventDefault();
        //$(document).scrollTop($('.publi-wrap').offset().top);
   ...
   // item.style.width = `${(times + 1) * width}px`
    ...
    itemEvent(item3);
    itemEvent(item4);
    //item2.addEventListener('mouseenter', () => {handleMouse(true)})
    //item2.addEventListener('touchstart', () => {handleMouse(true)})
    //item2.addEventListener('mouseleave', () => {handleMouse(false)})
    //item2.addEventListener('touchend', () => {handleMouse(false)})
